...
class Myurllib():
    ...
    def multi_get(self, url):
        nurl = urlparse.urlparse(url)
        if not self.sock:
            self.host = nurl.netloc
            self.get_sock()
        header = 'GET %s HTTP/1.0\r\n' % (nurl.path)
        header += 'Connection: keep-alive\r\n'
        header += 'Cache-Control: max-age=0\r\n'
        ...
        if self.sock:
            self.sock.send(header)
        # data = self.recv_timeout(self.sock)
        res = ''
        data = self.sock.recv(1024)
        while (s = self.sock.recv(1024)):
            res += data
        # while len(data) == 0:
        #     self.get_sock()
        #     self.sock.send(header)
...
# m.multi_get("http://cs5700sp15.ccs.neu.edu/")
